...
"""
    ...
    "review": {
        ...
        "activation": {
            "thinking": "--ultrathink",
            "mcp": ["Sequential", "Context7"],
            "flags": ["--focus quality", "--validate", "--scope project"],
            "persona": "analyzer",
        },
        "behavioral_rules": """
## REVIEW Mode Behavioral Rules
...
### Review Persona Sequence
순차적으로 다음 관점에서 분석:
1. **analyzer**: 현상태 객관적 분석
2. **security**: 보안 취약점 탐지
3. **performance**: 성능 병목점 식별
4. **architect**: 아키텍처 패턴 평가
5. **refactorer**: 리팩토링 기회 식별
6. **mentor**: 교육적 피드포워드 제공
...
- Critical Review (현상태 분석)
- Feedback (구체적 개선안)
- Feedforward (미래 지향적 제안)
- Action Items (우선순위별 할 일)
- Learning Opportunities (학습 기회)
""",
        "message": """
...
✓ 3-Phase Review Framework:
  ...
  • Phase 2: Feedback (구체적 개선안)
  • Phase 3: Feedforward (미래 지향적 제안)
✓ Multi-Persona Review Sequence:
  • analyzer → security → performance
  • architect → refactorer → mentor
✓ Severity Classification (Critical → Low)

📋 **Review Scopes:**
- `/review src/` : 코드 품질 리뷰
- `/review --scope arch` : 아키텍처 리뷰
- `/review --scope security` : 보안 중심 리뷰
- `/review --depth deep` : 심층 분석

💡 "Critique to elevate, not to diminish."
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
    },
}
